...
class NewEpisodeEmbed(Embed):
    def __init__(self, anime_name: str, episode: int, image_url: str):
        super().__init__(title=f"New episode for {anime_name} is available!", description=f"Episode {episode}", color=0x00ff00)
        ...
        async with AnitakuWrapper() as anitaku:
            ...
            for anime in anime_list:
                ...
                if latest_episode > last_episode:
                    ...
                    await asyncio.gather(*[self.send_new_episode_notification(channel[0], anime_name, latest_episode, image_url) for channel in channels])
